...
def consensus(all_values_event):
    ...
    while running:
        ...
        for nei in config.partners:
            ...
            Uw_1 += (abs(f_received[nei[0]] - f_received[config.me[0][0]]) ** gama) * sign(f_received[nei[0]] - f_received[config.me[0][0]])
                    # * config.A[nei_idx]
            Uw_2 += f_received[nei[0]] - f_received[config.me[0][0]]

            Ue += -k2 * (abs(Eta_received[nei[0]] - Eta_received[config.me[0][0]]) ** beta) * sign(Eta_received[nei[0]] - Eta_received[config.me[0][0]]) \
                  - k1 * (abs(SOC_received[nei[0]] - SOC_received[config.me[0][0]]) ** alpha) * sign(SOC_received[nei[0]] - SOC_received[config.me[0][0]])
                  # * config.A[nei_idx]

            Uq_1 += (abs(V_received[nei[0]] - V_received[config.me[0][0]]) ** gama) * sign(V_received[nei[0]] - V_received[config.me[0][0]])
                    # * config.A[nei_idx]
            Uq_2 += V_received[nei[0]] - V_received[config.me[0][0]]

        Uw_1 += (abs(f_ref - f_received[config.me[0][0]]) ** gama) * sign(f_ref - f_received[config.me[0][0]])
        # Uw_2 += (f_ref - f_meas)
        Uw = kw * (Uw_1 + Uw_2)

        Uq_1 += (abs(V_ref - V_received[config.me[0][0]]) ** gama) * sign(V_ref - V_received[config.me[0][0]])
        # Uq_2 += (V_ref - V_meas)
        Uq = kv * (Uq_1 + Uq_2)
        #send to opal
