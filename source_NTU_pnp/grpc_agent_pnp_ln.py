...
def consensus(all_values_event):
    ...
    while running:
        ...
        for nei in partners_list:
            Uw_1 += f_received[nei] - f_received[config.me[0][0]]

            Ue += -k2 * (Eta_received[nei] - Eta_received[config.me[0][0]]) \
                  - k1 * (SOC_received[nei] - SOC_received[config.me[0][0]])

            Uq_1 += V_received[nei] - V_received[config.me[0][0]]
